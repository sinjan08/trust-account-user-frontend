...
import { getAlLOutstandingDisbursements } from '../../../redux/slices/outstandingSlice';
...
const OutstandingDisbursements = () => {
    ...
    const { outstandingDisbursements, loading } = useSelector(state => state.outstanding);
    ...
    useEffect(() => {
        dispatch(getAlLOutstandingDisbursements());
    ...
    useEffect(() => {
        let filtered = [...outstandingDisbursements];
        ...
        if (searchTerm) {
            filtered = filtered.filter(item =>
                ...
                // item?.deposit_amount?.toString().toLowerCase().includes(searchTerm.toLowerCase()) ||
                // item?.disbursement_amount?.toString().toLowerCase().includes(searchTerm.toLowerCase()) ||
                // item?.running_balance?.toString().toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    }, [outstandingDisbursements, searchTerm, selectedMonthYear, startDate, endDate]);
    ...
    const uniqueMonthYears = [
        ...new Set(
            outstandingDisbursements?.map(item => {
    ...
    const csvRequiredKeys = ['date', 'client_name', 'payee_name', 'cheque_number', 'disbursement_amount'];

    const pdfData = selectedLiens.length > 0 ? selectedLiens : outstandingDisbursements;
    ...
    const handleDateCancel = () => {
        setFilteredData(outstandingDisbursements);
    };
                ...
                <div className="dashboard-body dsbrd-tbl-body">
                    <form>
                        <div className="ds-bdy-head mb-3">
                            <div className="ds-filter-wrp">
                                ...
                                <div className="bank-charges-inr-btn-wrp">
                                    <ReportDownloadDropdown name={'Download Report'} data={outstandingDisbursements} csvData={csvData} csvRequiredKeys={csvRequiredKeys} reportName={'Oustanding Disbursement Report'} />
                                </div>
                            </div>
                        </div>
                                ...
                                <table className="leader-summry-table" style={{ minWidth: 'max-content' }}>
                                    <thead>
                                        <tr>
                                            ...
                                            <th className="amount-col" onClick={() => handleSort("disbursement_amount")} style={{ cursor: "pointer" }}>Amount {getSortArrow("disbursement_amount")}</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {loading ?
                                            (
                                                ...
                                                paginatedData?.map((row, index) => (
                                                    <tr key={index}>
                                                        ...
                                                        <td className="text-truncate">{formatCurrency(row?.disbursement_amount)}</td>
                                                    </tr>
                                                ))
                                            ) : (
                                                <tr>
                        ...
                        <PaginationControl
                            ...
                            pageSize={perPage}
                            totalItems={outstandingDisbursements.length}
                            onPageChange={(page) => {
...
export default OutstandingDisbursements
